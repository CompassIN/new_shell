...
if [ -e /tmp/netstorm_build ] || [ -e /tmp/thirdparty_build ]
then
        rm /tmp/*_build
fi
if [ -e out_tp.log ] || [ -e out_ns.log ]
then
        rm out*.log
fi
...
tmp_files
echo -e "Enter the build version as shown in format:-\n(Build#version,418#2)\n"
read bv
